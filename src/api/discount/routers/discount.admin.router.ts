...
const discountAdminRouter = Router();

discountAdminRouter.post(
    "/",
    ...
    asyncWrapper(discountAdminController.createGlobalDiscount),
);

discountAdminRouter.get(
    "/:id",
    ...
    asyncWrapper(discountAdminController.getDiscount),
);

discountAdminRouter.patch(
    "/:id",
    ...
    asyncWrapper(discountAdminController.updateDiscount),
);

discountAdminRouter.delete(
    "/:id",
    rateLimit({ limit: 50, scope: "route", windowSeconds: 60 }),
    authenticator("access"),
    validator({ params: discountIdParamsSchema }),
    rbac([DISCOUNT_PERMISSIONS.DISCOUNT_DELETE]),
    asyncWrapper(discountAdminController.deleteDiscount),
);

export { discountAdminRouter };
